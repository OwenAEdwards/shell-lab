...
void eval(char *cmdline) {
	...
	if (!builtin_cmd(argv)) {
		sigemptyset(&mask);
		sigaddset(&mask, SIGCHLD); // SIGCHLD == if child is terminated
		sigprocmask(SIG_BLOCK, &mask, NULL); // then block
	
		if ((pid = fork()) == 0) { /* Child runs user job */
			sigprocmask(SIG_UNBLOCK, &mask, NULL); // unblocked
		...
		int state = bg ? BG : FG; // ternary operator -- bg if background, otherwise FG
		...
		// background job
			...
			printf("[%d] (%d) %s", job->jid, pid, cmdline); // [1] (1112) ./myspin 3 &
		}
